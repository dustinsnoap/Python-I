...
def titleMfrYearString (arr, index) :
    return f"{arr[index]['title']} was made by {arr[index]['manufacturer']} in {arr[index]['year']}"

def whoMadeWhatWhen(arr, mfrStr, cb):
    returned = []
    for i in range(len(arr)):
        if arr[i]['manufacturer'] == mfrStr:
            returned.append(cb(arr, i))

    return returned

# print(arcadeGames)
print(whoMadeWhatWhen(arcadeGames, 'Williams', titleMfrYearString))